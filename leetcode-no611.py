class Solution2:
    def triangleNumber(self, nums: list[int]) -> int:
        ...
        for i in range(nums.count(0)):
            nums.remove(0)
        def isok(p1,p2,p3):
            p1,p2,p3 = nums[p1],nums[p2],nums[p3]
            return (p1+p2>p3 and p2+p3>p1 and p1+p3>p2)
        res = 0
        right = len(nums)-1
        if right < 2:
            return 0
        ...
        while right >= 2:
            left = right - 2
            while left >= 0:
                if nums[left] == nums[left+1] and left + 1 < right - 1:
                    res += k + isok(left,left,right)
                    left -= 1
                    continue
                k = 0
                mid = right - 1
